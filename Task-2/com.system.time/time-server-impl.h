...
protected:
    uint64_t GetSystemTime() override {
     ...
     uint32_t pid;
        ...
        connection->callMethod("CheckApplicationHasPermission").onInterface(serviceName).withArguments(execPath, 0).storeResultsTo(hasPermission);

        if(!hasPermission) {
            throw sdbus::Error(sdbus::Error::Name{"com.system.time.Error"}, "Unauthorized access");
        }
