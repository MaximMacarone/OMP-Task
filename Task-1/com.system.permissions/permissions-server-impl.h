...
protected:
  ...
  CheckApplicationHasPermission(const std::string &applicationExecPath,
    ...
    const char *selectSQL = "SELECT COUNT(*) FROM Permissions WHERE ExecPath = "
                            "? AND PermissionCode = ?;";
    ...
    if (sqlite3_prepare_v2(db_, selectSQL, -1, &stmt, nullptr) == SQLITE_OK) {
      ...
      std::cerr << "Failed to prepare SQL statement: " << sqlite3_errmsg(db_)
                << std::endl;
    }
...
private:
  ...
  void savePermissionRequest(pid_t clientPID, const std::string &execPath,
                             const int32_t &permissionCode) {
    const char *insertSQL = "INSERT INTO Permissions (ClientPID, ExecPath, "
